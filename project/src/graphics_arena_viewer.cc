...
void GraphicsArenaViewer::DrawEntity(NVGcontext *ctx,
  ...
  if (entity->get_type() == kPredator) {
    auto predator = static_cast<const Predator * const>(entity);
    std::vector<Pose> sensors = predator->get_light_sensors_const();
    Pose left_sens_pose = sensors[0];
    Pose right_sens_pose = sensors[1];
    // left sensor drawing save the ctx twice, once for the circle and
    // once for the text
...
void GraphicsArenaViewer::AddEntityPanel(nanogui::Widget * panel) {
  ...
  oldentity = static_cast<BraitenbergVehicle*>(defaultEntity);
  ...
  robotPanel->setLayout(new nanogui::BoxLayout(
    nanogui::Orientation::Vertical, nanogui::Alignment::Minimum, 0, 0));
  nanogui::ComboBox* bvBehaviorSelect =
    new nanogui::ComboBox(robotPanel, behaviorNames);
  ...
  grid->setLayout(new nanogui::GridLayout(
    nanogui::Orientation::Horizontal, 3,
  ...
  if (defaultEntity->get_type() == kBraitenberg) {
    ...
    if (oldentity != NULL) {
      oldentity->Unsubscribe();
    }
    static_cast<BraitenbergVehicle*>(defaultEntity)->Subscribe(this);
    static_cast<BraitenbergVehicle*>(defaultEntity)->Notify(this);
    oldentity = static_cast<BraitenbergVehicle*>(defaultEntity);
  }
  ...
  entitySelect->setCallback(
    ...
    foodBehaviorSelect, bvBehaviorSelect](int index) {
      ...
      if (entity->get_type() == kBraitenberg) {
        ...
        static_cast<BraitenbergVehicle*>(entity)->Notify(this);
      }
      screen()->performLayout();
    });
...
void GraphicsArenaViewer::OnUpdate(
  WheelVelocity wv1, WheelVelocity wv2, WheelVelocity wv3) {
  if (light_value_left_ != NULL) {
    light_value_left_->setValue(std::to_string(wv1.left).substr(0, 3));
  }
  if (light_value_right_ != NULL) {
    light_value_right_->setValue(std::to_string(wv1.right).substr(0, 3));
  }
  if (food_value_left_!= NULL) {
    food_value_left_->setValue(std::to_string(wv2.left).substr(0, 3));
  }
  if (food_value_right_ != NULL) {
    food_value_right_->setValue(std::to_string(wv2.right).substr(0, 3));
  }
  if (bv_value_left_ != NULL) {
    bv_value_left_->setValue(std::to_string(wv3.left).substr(0, 3));
  }
  if (bv_value_right_ != NULL) {
    bv_value_right_->setValue(std::to_string(wv3.right).substr(0, 3));
  }
}
