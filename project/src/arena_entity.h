 ...
 */
  ...
  virtual void LoadFromObject(json_object* entity_config) {
    ...
    set_position(
      e_config["x"].get<double>(), e_config["y"].get<double>());
    if (e_config.find("theta") != e_config.end()) {
      pose_.theta = e_config["theta"].get<double>();
    }
    if (e_config.find("r") != e_config.end()) {
      set_radius(e_config["r"].get<double>());
    }
  }

  virtual ArenaEntity* get_internal_entity() { return NULL; }

  // virtual void set_behavior() = 0;
